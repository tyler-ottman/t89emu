...
uint32_t RAMMemoryDevice::read(uint32_t addr, uint32_t size, uint32_t* read_value) {
...
uint32_t RAMMemoryDevice::write(uint32_t addr, uint32_t value, uint32_t size) {
    // Check memory access is aligned
    if (!check_alignment(addr, size)) {
