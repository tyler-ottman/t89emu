...
uint32_t NextPc::calculateNextPc(uint32_t offset, uint32_t opcode,
                                 uint32_t funct3, uint32_t A, uint32_t B,
                                 uint32_t mepc) {
    switch (opcode) {
    case JAL:
        nextPc += offset;
    ...
    case JALR:
        ...
        break;  // JALR signal
    case BTYPE:
        nextPc += branchAlu(A, B, funct3) ? offset : 4;
        break;  // B-type signal
    case PRIV:
