...
extern int get_tp(void);
extern void pushThread(TThreadID);

// TCB to store information about threads
struct TCB {
    TThreadEntry entry;
    void *param;
    TMemorySize memsize;
    TThreadPriority prio; // HIGH = 3, NORMAL = 2, LOW = 1, IDLE = 0
    TThreadID tid;
    uint32_t *stackPointer;
    uint8_t* baseOfStack;
    uint32_t state; // Created = 0, Read = 1, Running = 2, Dead = 3, Waiting = 4
    uint32_t isEmpty;
    TThreadID waitingOn; // If thread is waiting for another thread to terminate
    TThreadReturnRef returnValueRef; // Reference of returned value from thread being waited on
    TThreadReturn returnValue; // Return value of this thread
    TTick ticksUntilReady; // Keep track of ticks until sleeping thread wakes
};
