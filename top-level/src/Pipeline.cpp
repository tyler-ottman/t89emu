...
bool Pipeline::OnUserUpdate(float fElapsedTime)
{
	...
	if (this->debug_mode)
	{
		debug.debug_pre_execute(opcode, funct3, funct7, rs1, rs2, rd, immediate, csr_addr, cur_instruction);
	}
	/******************DEBUG********************/
	...
	if (this->debug_mode)
	{
		debug.debug_post_execute(opcode, rd, immediate, rf.read(rd), rf.read(rs2), rf.read(rs1), pc.getPC());
	}
	/******************DEBUG********************/
